"""Qwen VL Vision provider implementation using dashscope SDK."""
...
class QwenVisionProvider(BaseVisionProvider):
    """Vision analysis using Alibaba Cloud Qwen VL model."""
    ...
    async def analyze_image(
        ...
        **kwargs,
        ...
        """Analyze image using Qwen VL via HTTP API."""
        try:
            import httpx
            ...
            if "choices" in data and len(data["choices"]) > 0:
                ...
                return VisionAnalysis.failure(
                    error=f"Qwen API unexpected response: {data}",
                    provider=self.provider_name,
                )
            ...
            return VisionAnalysis.failure(
                error=f"Qwen Vision error: {exc!s}",
                provider=self.provider_name,
            )
